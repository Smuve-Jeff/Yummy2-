...
if (typeof window.process === 'undefined' || window.process === null) {
  ...
  console.warn("main.ts: Forcibly initialized window.process.");
  ...
  console.warn("main.ts: Replaced/initialized window.process.env object to ensure mutability and extensibility.");
}
...
if (currentApiKeyString.toLowerCase() === 'undefined' || currentApiKeyString === '' || currentApiKeyString.length < 30) {
  ...
  console.warn(`main.ts: Detected problematic API_KEY in window.process.env (original value: "${originalApiKeyFromEnv}"). Setting to empty string.`);
} else {
  finalSanitizedApiKey = currentApiKeyString;
  console.log(`main.ts: Valid API_KEY detected from window.process.env (length: ${finalSanitizedApiKey.length}).`);
}
...
console.log(`main.ts: window.process.env.API_KEY is now explicitly set to a sanitized value (length: ${((window.process as any).env.API_KEY as string).length}).`);
...
if (window.AURA_GEMINI_API_KEY === '') {
  console.warn("main.ts: Final AURA_GEMINI_API_KEY for application use is empty. AI features will be disabled.");
} else {
  console.log(`main.ts: AURA_GEMINI_API_KEY is available for application use (length: ${window.AURA_GEMINI_API_KEY.length}).`);
}
...
import { API_KEY_TOKEN } from './src/tokens'; // Import API_KEY_TOKEN from new file
